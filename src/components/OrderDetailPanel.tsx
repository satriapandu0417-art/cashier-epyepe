    ...
    <>
      ...
      >
        ...
        >
          ...
          {/* Items List */}
          <div className={`flex-1 overflow-y-auto p-6 space-y-3 ${!isEditing && (isPickedUp || order.status === 'Cancelled') ? 'opacity-75 pointer-events-none grayscale-[0.5]' : ''}`}>
            {isEditing ? (
              <>
                ...
                <AnimatePresence>
                  {editedItems.map((item) => (
                    <motion.div
                      ...
                      className="p-4 rounded-xl border border-orange-100 bg-orange-50/30"
                    >
                      <div className="flex justify-between items-start mb-3">
                        <div>
                          <h4 className="font-bold text-gray-900">{item.name}</h4>
                          <p className="text-xs text-gray-500">{formatCurrency(item.basePrice)} / unit</p>
                        </div>
                        <button
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3 bg-white border border-orange-100 rounded-lg p-1">
                          <button
                      ...
                      <div className="flex-1 overflow-y-auto p-4 space-y-2">
                        {filteredMenu.map(item => (
                          <button
                            ...
                            className="w-full flex items-center justify-between p-3 rounded-xl border border-gray-100 hover:border-orange-200 hover:bg-orange-50 transition-all text-left"
                          >
                            <div>
                ...
                <div className="space-y-3">
                  {order.editHistory.map((log, idx) => (
                    <div key={idx} className="bg-gray-50 p-3 rounded-xl border border-gray-100">
                      ...
                      <p className="text-xs text-gray-600 leading-relaxed">{log.changes}</p>
                    </div>
                  ))}
                </div>
              </div>
          ...
          <div className="p-6 border-t border-gray-100 bg-gray-50 space-y-6">
            <div className="space-y-2">
              ...
              <div className="flex justify-between items-center pt-3 border-t border-gray-200">
                ...
                <span className="text-2xl font-black text-orange-600">{formatCurrency(isEditing ? editedTotal : order.total)}</span>
              </div>
            </div>
