...
import { X, CheckCircle2, Clock, ChefHat, AlertCircle, ShoppingBag, Lock, Trash2, XCircle, Tag } from 'lucide-react';
...
export function OrderDetailPanel({ order, onClose }: OrderDetailPanelProps) {
  const { toggleOrderItemPrepared, updateOrderStatus, updateOrderPaymentStatus, deleteOrder } = useStore();
    ...
    <>
      ...
      >
        ...
        >
          ...
          {/* Items List */}
          <div className={`flex-1 overflow-y-auto p-6 space-y-3 ${isPickedUp || order.status === 'Cancelled' ? 'opacity-75 pointer-events-none grayscale-[0.5]' : ''}`}>
